...
-- 댓글 삭제시
delimiter //
create trigger reply_count_down
	after delete
	on boardReply
	for each row
begin
...
-- 답글 작성시
delimiter //
create trigger reReply_count_up
	after insert
	on boardrereply
	for each row
begin
...
-- 답글 삭제시
delimiter //
create trigger reReply_count_down
	after delete
	on boardrereply
	for each row
begin
...
-- 좋아요 해제 시
delimiter //
create trigger good_count_down
	after delete
	on board_good
	for each row
begin
